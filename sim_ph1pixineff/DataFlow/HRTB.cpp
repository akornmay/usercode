...
HRTB::HRTB() : READOUT(0), gap(0), header(0), trailer(0), ro_pix(0), ro_clocks(0)
{
	triggerStack.clear();
}
...
void HRTB::Clock()
{
	...
	if(*bx_counter==(WBC+5+TS)){                            // sets TS = timeStamp if expired, 0 otherwise
	  ...
	  if(iRead==TELESCOPE_STACK_SIZE) iRead=0;
	} else TS=0;
   ...
   if(READOUT){                                            // token scan running
      ...
      if(!token->Readout(RO_TS)){                          // r/o finished for this ROC
         ...
         if(token==last_token) 	{                         // end of readout
           trailer=TELESCOPE_TRAILER_LENGTH;                     // start TBM trailer
      ...
      header=TELESCOPE_HEADER_LENGTH;
